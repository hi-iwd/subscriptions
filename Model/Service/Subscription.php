...
class Subscription implements SubscriptionManagementInterface
{
    ...
    public function __construct(
		...
		\IWD\BluePaySubs\Helper\Log $log
    )
    {
    ...
    public function synchronize(SubscriptionInterface $subscription, $agentId = 0)
    {
        ...
        try {
            ...
            if ($rebillStatus->getStatus() == Status::STATUS_PAYMENT_FAILED or $rebillStatus->getStatus() == Status::STATUS_ERROR) {
    ...
    {
		$subs = $this->_getSaledRebillsFaild($subscription);

        foreach ($subs as $sub){
            if($sub->getStatus() == 1){
                $subscription->setPaymentFailedRunCount(0);
            }
        }
    ...
    protected function _updateSubscriptionPaymentFailedCount(SubscriptionInterface $subscription)
    {
        ...
        } elseif ($subscription->getStatus() == Status::STATUS_PAYMENT_FAILED or $subscription->getStatus() == Status::STATUS_ERROR) {
    ...
    protected function _reactivateSubscription(SubscriptionInterface $subscription, $agentId)
    {
		...
		$failedRunCount = $this->helper->getPaymentFailedRunCount();
        $nextRunTime = strtotime(
            sprintf('+%s %s', $retryPeriod, Period::PERIOD_DAY),
            strtotime($lastRunDate)
        );
        if ($subscription->getPaymentFailedRunCount() >= $failedRunCount) {
            return $this;
        }
        ...
        try {
            ...
            foreach ($payedRebills as $rebill) {
                ...
                if (empty($orderPayments)) {
                    ...
                    try {
                        if($this->log->getTransaction($rebill->getData('id'))){
                            $this->_generateOrderInternal($subscription, $quote,$rebill);
                        }
                    } catch (\Exception $e) {
		...
		if(count($rebillSaleData) > 0){
			foreach ($rebillSaleData as $rebill) {
				...
				if (empty($rebill->getData('status'))) {
					...
					$payedRebills[$rebill->getData('rebill_id')] = $rebill;
				}
			}
		}
	...
	protected function _getSaledRebillsFaild(SubscriptionInterface $subscription)
    {
        $payedRebills = [];
        $lastDate = $subscription->getOrigData(SubscriptionInterface::LAST_DATE);
        if (empty($lastDate)) {
    ...
    )
    {
        ...
        $subscription->recordBilling($order,$message,$rebill);
